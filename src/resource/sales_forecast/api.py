...
from src.functionality.sales_forecast.sales_forecast import (
    predict_sales,
    predict_next_30_days_sales,
...
class SalesInfoResponse(BaseModel):
    sales: dict
...
def predict_sales_api(org_id: str, prediction_date: PredictionRequest):
    ...
    return SalesInfoResponse(sales=sales_info)
...
@sales_router.post("/train_sales_model/", response_model=SalesPredictionResponse)
def train_sales_model_api(organization_id: str):
    sales_info = train_sales_prediction_model(organization_id)
    return sales_info  # Pydantic model will handle serialization
