...
from src.functionality.customer.customer import (
    create_customer, update_customer, get_customer, delete_customer,get_all_customers,get_all_customers_name
)
...
@customer_router.get("/{org_id}/customers/names", status_code=200)
def get_all_customer_name_api(org_id: str, 
    user_data: Annotated[dict, Depends(authorization)]
):
    user_id = user_data.get("user_data").get("id")
    # org_id = user_data.get("user_data").get("organization_id")
    response = get_all_customers_name(org_id, user_id)
    return response
